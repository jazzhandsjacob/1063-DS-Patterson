...
class avlTree {
	...
	struct node {
		string value, valtype;
		node *left;
		node *right;
		node *parent;
		int avlValue;
		node(string word, string type) {
			value = word;
			valtype = type;
...
private:
	...
	//
	// Arguments: passes in the new value you and compares it to the value
	//			stored in the tree and moveint into the correct order.
	//
	// Returns: Nothing.
	//*************************************************************************
	...
	//
	// Arguments: address of a node and an empty string
	//
	// Returns: void 
	//*************************************************************************
	void printNode(node *n, string label = "")
	{
		if (label != "")
		{
			cout << "[" << label << "]";
	...
	void search(string word) {
		...
		while (nodePtr) {
			if (nodePtr->value == word)
			{
				...
				if (nodePtr->valtype == "adj")
					adjcount++;
				else if (nodePtr->valtype == "adv")
					advcount++;
				else if (nodePtr->valtype == "animals")
					animalcount++;
				else if (nodePtr->valtype == "noun")
					nouncount++;
				else 
					verbcount++;
				return;
