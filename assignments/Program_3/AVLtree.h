...
bool avlTree::search(string word) {
	...
	while (nodePtr) {
		//+1  comparison counter for doing a traversal
		if (nodePtr->value == word)
			//comparison to increment correct counter
			//if (valtype == "adjective)
			//increment adjective counter
			return true;
		else if (word < nodePtr->value)
			nodePtr = nodePtr->left;
		//+1 counter for doing a traversal
		else
